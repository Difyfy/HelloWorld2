...
/// An entity that represents Uranus and its configuration.
class UranusEntity: Entity {

    // MARK: - Sub-entities

    /// The model that draws Uranus's surface features.
    private var uranus: Entity = Entity()

    // MARK: - Initializers

    /// Creates a new blank Uranus entity.
    @MainActor required init() {
        super.init()
    }

    /// Creates a new Uranus entity with the specified configuration.
    ///
    /// - Parameters:
    ///   - configuration: Information about how to configure Uranus.
    init(configuration: Configuration) async {
        super.init()

        // Load the Uranus model.
        guard let uranus = await WorldAssets.entity(named: "Uranus") else { return }
        self.uranus = uranus

        // Attach the Uranus model to the entity.
    ...
    func update(configuration: Configuration, animateUpdates: Bool) {
        // Set the rotation of Uranus.
        uranus.orientation = configuration.rotation

        // Set the speed of Uranus's rotation on its axis.
        if var rotation: RotationComponent = uranus.components[RotationComponent.self] {
            rotation.speed = configuration.currentSpeed
            uranus.components[RotationComponent.self] = rotation
        } else {
            uranus.components.set(RotationComponent(speed: configuration.currentSpeed))
        }
    ...
    /// Create an accessibility component suitable for the Uranus entity.
    ///
    /// - Parameters:
    ///   - configuration: Information about how to configure Uranus.
    ...
    private func makeAxComponent(configuration: Configuration) -> AccessibilityComponent {
        ...
        // Add a label.
        axComponent.label = "Uranus model"
